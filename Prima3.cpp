...
int main() // ¬вод графа
{
   ...
   else
   {
	  ...
	  cout << "4 - разр€женный граф(матрица смежности)" << endl;
	  cout << "5 - насыщенный граф(список смежности)" << endl;
	  cout << "6 - разр€женный граф(список смежности)" << endl;
	  cin >> num;
	  switch (num)
	  {
	  case 1:
	  {
		 fin >> maxSize; // количество рЄбер
		 while (!fin.eof())
		 {
...
void DenseGraphGeneration(int graphSize)
{
   ...
   for (int i = 0; i < graphSize; i++)
   {
	  ...
	  {
		 if (i == 0 && j == 0) // выполн€етс€ один раз
		 {
			cur = 1 + rand() % (graphSize - 1);
			vecForMatrix[i][cur] = 100 + rand() % 100; // сделать чтобы соедин€ло с рандомной
		 }
		 else
		 {
			if (i == j || i == 0 && j != cur)
